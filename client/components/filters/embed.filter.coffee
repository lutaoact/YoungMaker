  ...
  (input) ->
    ...
    result = switch true
      ...
      when /v.youku.com\/(.*)id_(.*).html/.test url
        ...
        '<iframe height=250 width=320 src="http://player.youku.com/embed/' + url.replace(/.*id_(.*).html.*/,'$1') + '" frameborder=0 allowfullscreen></iframe>'
      when /v.qq.com\/cover(.*)vid=(.*)/.test url
        # qq video http://v.qq.com/cover/k/kd2yl4blio5xn17.html?vid=c0140yh6ew3
        '<iframe height=250 width=320 frameborder=0 src="http://v.qq.com/iframe/player.html?vid=' + url.replace(/.*vid=(.*)/,'$1') + '&tiny=0&auto=0" allowfullscreen></iframe>'
      else
        undefined
